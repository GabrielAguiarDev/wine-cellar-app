...
5. **Shared element da curadoria — calibrar no device:** durações (`DURACAO_ABERTURA` 560ms / `DURACAO_FECHAMENTO` 420ms), curva (`CURVA`), pontos de crossfade (`FIM_FADE_FANTASMA` 0.28 / `INICIO_FADE_CONTEUDO` 0.45) e `OPACIDADE_SCRIM` em `BlocoCuradoria.tsx`. Validado no simulador iOS (frame a frame). Conferir no Android se o `measureInWindow` bate com a janela edge-to-edge (a forma deve nascer exatamente sobre o card, sem salto).
6. Persistência de **onboarding já feita** (AsyncStorage). Falta persistir carrinho/favoritos, se desejado (mesmo padrão `persist`).
...
- **Shared element (curadoria)**: só funciona porque a rota `curadoria/[id]` está declarada no `app/_layout.tsx` como `presentation: 'transparentModal'` + `animation: 'none'` + `gestureEnabled: false` — a Home fica montada/visível por baixo e a Stack não anima por cima. Se essa rota virar push normal, a transição some.
