...
> **Última atualização: Atalhos da Home por país (bandeiras) no lugar de "Explorar por tipo" — 2026-07-29**
...
- **`src/data/`** (barrel `@data`): `types.ts` (Wine/Review/QuizQuestion/Occasion), `wines.ts` (WINES, 10), `reviews.ts` (REVIEWS), `quiz.ts` (QUIZ, 3), `occasions.ts` (OCCASIONS, 4), `curations.ts` (`CURATIONS`/`WEEKLY_CURATION`/`findCuration` — fonte única do conteúdo do `CurationBlock` nos dois estados), `selectors.ts` (`findWine`, `railSelected`, `railBestSellers`, `specials`, `countByType`, `countriesWithCount`, `winesByIds`, `searchWines`, `searchByDish`, `cartCount`, `cartSubtotal`, `CAT_SPECIALS`), `notifications.ts` (`NOTIFICATIONS` + tipos `AppNotification`/`NotificationKind`/`NotificationTarget`/`NotificationGroup`, `NOTIFICATION_GROUPS` e os seletores de não lidas/seção — ficam aqui, e não em `selectors.ts`, para não criar ciclo de import).
